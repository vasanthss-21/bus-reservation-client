...
function CancelBooking({ onBack }) {
  ...
  const handleCancellation = async (e) => {
    ...
    if (!customerName || !routeId || !travelTime || !seatNumber) {
      setError('Please provide Customer Name, Route ID, Travel Time, and Seat Number.');
      return;
    }
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-md max-w-lg mx-auto">
      ...
      <form onSubmit={handleCancellation}>
        ...
        <div className="mb-4">
          <label htmlFor="travelTime" className="block text-gray-700 font-medium mb-2">Travel Time</label>
          <input
            type="text"
            id="travelTime"
            value={travelTime}
            ...
            className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
            placeholder="Enter the travel time (e.g., 09:00)"
          />
        </div>

        <div className="mb-4">
          <label htmlFor="seatNumber" className="block text-gray-700 font-medium mb-2">Seat Number</label>
          <input
            type="number"
            id="seatNumber"
            value={seatNumber}
